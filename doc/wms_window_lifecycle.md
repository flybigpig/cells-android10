...
## 八、Session.java 完整属性与方法分析

`Session` 继承 `IWindowSession.Stub`（68 行），是 WMS 与应用进程之间跨进程会话的 Binder 实现。每个与 WMS 交互的进程一般对应一个 Session，由 `WindowManagerService.openSession`（WMS 5041 行）创建，客户端经 `WindowManagerGlobal.getWindowSession()` 拿代理。
...
**身份与权限（69-102 行）**
- `mService`：`WindowManagerService` 引用，所有实际操作转交它。
- `mCallback`：`IWindowSessionCallback`，客户端会话回调 Binder，构造时 `linkToDeath`（123 行）。
- `mUid` / `mPid`：调用者身份，构造时从 `Binder.getCallingUid/Pid()` 取（94-95 行），用于权限与记账。
- `mCanAddInternalSystemWindow`：缓存 `INTERNAL_SYSTEM_WINDOW` 权限（97 行），决定是否可加系统签名级窗口。
- `mCanHideNonSystemOverlayWindows`：缓存 `HIDE_NON_SYSTEM_OVERLAY_WINDOWS`（99 行）。
- `mCanAcquireSleepToken`：缓存 `DEVICE_POWER`（101 行）。
- `mPackageName`：首个窗口加入时由 `windowAddedLocked` 赋值（473 行）。
- `mRelayoutTag`：首个窗口加入时赋值（474 行），用于 `Trace` 标签。

**资源（74-80 行）**
- `mSurfaceSession`：本 Session 唯一的 `SurfaceSession`，同进程所有窗口共享，向 `SurfaceFlinger` 申请 Surface 的入口。首次加窗口时建（475-478 行），计数归零才释放（killSessionLocked 571 行）。
- `mDragDropController`：拖拽控制器引用（80 行），`performDrag`/`reportDropResult` 等委托它。

**记账集合（76-79 行）**
- `mNumWindow`：本 Session 窗口数；`windowAddedLocked` +1、`windowRemovedLocked` -1，归零后清理。
- `mAppOverlaySurfaces`：可见的 application overlay Surface 集合。
- `mAlertWindowSurfaces`：可见的 `SYSTEM_ALERT_WINDOW` 悬浮窗 Surface 集合，用于用户通知。

**状态/标志（86-89 行）**
- `mClientDead`：进程是否已死（binderDied 置 true）。
- `mShowingAlertWindowNotificationAllowed`：是否允许弹悬浮窗通知（WMS 全局配置决定）。
- `mAlertWindowNotification`：悬浮窗通知对象，延迟创建（519 行）。
- `mLastReportedAnimatorScale`：上一次动画缩放，变化时 `dispatchNewAnimatorScaleLocked`（482 行）。

### 8.2 方法清单（按职责分组）

**生命周期**
- `binderDied()`（145 行）：进程死亡回调，`unlinkToDeath` + 置 `mClientDead` + `killSessionLocked`。
- `windowAddedLocked()`（472 行）：首个窗口到来时建 `mSurfaceSession` 并把自身加入 `mService.mSessions`；`mNumWindow++`。
- `windowRemovedLocked()`（489 行）：`mNumWindow--` 后调 `killSessionLocked`。
- `killSessionLocked()`（557 行）：`mNumWindow==0 且 mClientDead` 才真正清理——`mService.mSessions.remove(this)`、销毁 `mSurfaceSession`、`clear` 两套悬浮窗集合、复位 overlay 状态、取消通知。

**窗口增删改（均为 Binder 入口，转交 WMS）**
- `addToDisplay`（154）/ `addToDisplayWithoutInputChannel`（165）→ `mService.addWindow`。
- `remove`（175）→ `mService.removeWindow`。
- `relayout`（185）→ `mService.relayoutWindow`（包 `Trace` 测速）。
- `prepareToReplaceWindows`（180）→ `setWillReplaceWindows`，配合无缝替换窗口机制。
- `finishDrawing`（228）→ `finishDrawingWindow`，首帧绘制完成上报。

**输入 / Insets**
- `setInsets`（216）、`getDisplayFrame`（223）、`insetsModified`（461）、`setInTouchMode`（235）/ `getInTouchMode`（242）、`updatePointerIcon`（431）、`updateTapExcludeRegion`（451）。

**拖拽与任务移动**
- `performDrag` / `reportDropResult` / `cancelDragAndDrop` / `dragRecipient*` （262-303）→ `mDragDropController`。
- `startMovingTask` / `finishMovingTask`（306/318）→ `mTaskPositioningController`。

**壁纸**
- `setWallpaperPosition` / `wallpaperOffsetsComplete` / `setWallpaperDisplayOffset` / `sendWallpaperCommand` / `wallpaperCommandComplete`（347-401），统一经私有 `actionOnWallpaper`（340）取 `WallpaperController` 后委托。

**悬浮窗可见性记账**
- `onWindowSurfaceVisibilityChanged`（495）：窗口 Surface 可见性变化时，按 `type` 增删 `mAlertWindowSurfaces`（`SYSTEM_ALERT_WINDOW` 类，弹/撤 `AlertWindowNotification`）或 `mAppOverlaySurfaces`（TYPE_APPLICATION_OVERLAY，通知 AMS 调整进程重要性，影响内存回收优先级）。
- `setShowingAlertWindowNotificationAllowed`（546）：全局开关悬浮窗通知。

**杂项**
- `performHapticFeedback`（249）→ `mPolicy.performHapticFeedback`；`outOfMemory`（206）、`setTransparentRegion`（211）、`reportSystemGestureExclusionChanged`（331）、`pokeDrawLock`（421）、`getWindowId`（416）、`setHasOverlayUi`（583，经 `mService.mH` 发 `SET_HAS_OVERLAY_UI` 消息给 AMS）。

**调试/工具**
- `dump`（595）：输出 `mNumWindow` / 两套悬浮窗集合 / `mClientDead` / `mSurfaceSession` 等状态。
- `toString`（606）：返回预构建的 `mStringName`。

### 8.3 设计要点

Session 是**薄 Binder 代理 + 进程级资源/权限账户**：自身不存窗口树状态，只负责(1) 把跨进程调用转交 WMS，(2) 维护进程唯一的 `SurfaceSession` 与窗口计数，(3) 记账悬浮窗 Surface 供用户通知与 AMS 重要性评分，(4) 监听进程死亡做清理（`killSessionLocked` 的 `mNumWindow==0 && mClientDead` 双条件保证不误删）。真正的窗口对象、层级树、token 都在 WMS / WindowState / WindowToken 一侧——这正是它与层级树"正交"的原因：它按进程维度存在，不进入 `DisplayContent → WindowToken → WindowState` 树。
