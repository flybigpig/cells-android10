...
module_param_named(debug_mask, binder_debug_mask, uint,
0644);
...
module_param_call(stop_on_user_error, binder_set_stop_on_user_error,
        param_get_int,
&binder_stop_on_user_error, 0644);
...
static struct binder_work *binder_dequeue_work_head_ilocked(
    ...
    w = list_first_entry_or_null(list,
...
static void dump_ref_desc_tree(struct binder_ref *new_ref, struct rb_node *n_in) {
    ...
    for (n = rb_first(&proc->refs_by_desc); n != NULL; n = rb_next(n)) {
        ...
        struct binder_ref, rb_node_desc);
        if (n->rb_left)
            left = rb_entry(n->rb_left,
        struct binder_ref,
        rb_node_desc);
        if (n->rb_right)
            right = rb_entry(n->rb_right,
        struct binder_ref,
        rb_node_desc);
        p = rb_parent(n);
        if (p)
...
if (
put_user(node_cookie, (binder_uintptr_t
__user *)ptr))
return -
EFAULT;
ptr += sizeof(binder_uintptr_t);
...
static void print_binder_proc(struct seq_file *m,
    ...
    binder_inner_proc_lock(proc);
    for (n = rb_first(&proc->threads); n != NULL; n = rb_next(n))
        print_binder_thread_ilocked(m, rb_entry(n,
    struct binder_thread,
    rb_node), print_all);
...
static void print_binder_proc_stats(struct seq_file *m,
    ...
    binder_proc_lock(proc);
    for (n = rb_first(&proc->refs_by_desc); n != NULL; n = rb_next(n)) {
        struct binder_ref *ref = rb_entry(n,
        struct binder_ref,
        rb_node_desc);
        count++;
